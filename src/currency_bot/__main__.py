...
from .config import USER_PREFERENCES, DEFAULT_TARGETS, SUPPORTED_CURRENCIES, save_preferences
...
def config_currencies(message):
    ...
    if message.chat.id not in USER_PREFERENCES:
        USER_PREFERENCES[message.chat.id] = {"targets": list(DEFAULT_TARGETS), "format": "international"}
    ...
    if not args:
        bot.reply_to(message, f"Current Currencies: {', '.join(current)}\nFormat: {current_fmt}\n\nUse /help to see how to change these.")
        return
    ...
    if action not in ['set', 'add', 'remove'] or not currencies:
        bot.reply_to(message, "Please provide an action and currency codes. Example: /config add USD")
        return
    ...
    if action == 'set':
        ...
        bot.reply_to(message, f"Your output currencies have been set to: {', '.join(prefs['targets'])} (max 3)")
    elif action == 'add':
        for c in currencies:
            if c in current:
                current.remove(c)
        ...
        bot.reply_to(message, f"Your output currencies have been updated to: {', '.join(prefs['targets'])} (max 3)")
    elif action == 'remove':
        for c in currencies:
            if c in current:
                current.remove(c)
        if not current:
            ...
            bot.reply_to(message, f"Cannot remove all currencies. Resetting to default: {', '.join(current)}")
        else:
            bot.reply_to(message, f"Your output currencies have been updated to: {', '.join(current)}")
...
def handle_message(message):
    ...
    for amount, currency in matches:
        ...
        if rates:
            conversion_text = calculate_conversions(amount, currency, rates, message.chat.id)
            if conversion_text:
...
if __name__ == "__main__":
    if init_supported_currencies():
        ...
        logger.error("Failed to initialize supported currencies.")
